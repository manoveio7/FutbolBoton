...
// --- CAMBIO MULTIJUGADOR: Conectar al servidor ---
// Reemplaza la URL con la que te asigne Render (ej: 'https://mi-juego.onrender.com')
var socket = io('https://futbol-server-x70l.onrender.com');
var main = {
	key: 'main',
	active: false,
	create: inicio,
...
function inicio() {
	...
	NOM = ['RUBEN','JULIO','SILVIO','FABRICIO','ALBERTO','DANIEL','ERICO','EDGARDO']
	LIMITADORES = [];
	estaEscena = this;
	...
	fondo = this.add.image(0, 0, 'fondo').setOrigin(0).setDisplaySize(cam.width, cam.height)
    ...
    // sensor de gol...
	let gol1 = this.physics.add.image(50, 280, 'cuadro').setImmovable(true).setName('gol1').setOrigin(0, 0).setAlpha(0).setDisplaySize(cam.width / 20, cam.height / 3.5).setTint(0xffff01)
	let gol2 = this.physics.add.image(cam.width - 50, 280, 'cuadro').setImmovable(true).setName('gol2').setOrigin(1, 0).setAlpha(0).setDisplaySize(cam.width / 20, cam.height / 3.5).setTint(0xffff01)
	...
	for (let i = 0; i < 8; i++) {
		...
		switch (i) {
			case 0: jug.x = 580; jug.y = 375; break;
			case 1: jug.x = 450; jug.y = 535; break;
			case 2: jug.x = 450; jug.y = 225; break;
			case 3: jug.x = 250; jug.y = 375; break;
			case 4: jug.x = 920; jug.y = 375; break;
			case 5: jug.x = 1250; jug.y = 375; break;
			case 6: jug.x = 1080; jug.y = 535; break;
			case 7: jug.x = 1080; jug.y = 225; break;
		}
		jug.posIniX = jug.x; jug.posIniY = jug.y;
		jug.fau = false; jug.i = i;
		if (i < 4) jug.equipo = 'A'; else jug.equipo = 'B';
		jugA.add(jug);

		let res = this.add.image(jug.x, jug.y, 'resplandor').setDisplaySize(cam.width / 10, cam.width / 10).setDepth(1).setAlpha(0);
		RESPLANDOR.add(res);

		let fJug = this.add.image(i < 4 ? 10 : cam.width - 125, 10, 'f' + i).setOrigin(0).setDepth(110).setDisplaySize(118, 95).setAlpha(0)
		fJUG.add(fJug);
	}

	jugA.children.iterate(function (player) {
		player.setDisplaySize(cam.width / 15, cam.width / 15)
		player.setInteractive().setDepth(1).setBounce(0.5).setDrag(0.1).setDamping(true).setFriction(0.05).setCollideWorldBounds();
		player.body.isCircle = true
		player.on('pointerdown', tocando);
	});
	
	jues = this.add.image(cam.width/2,-100,'jues').setScale(equipoA === true ? 0.8 : -0.8,0.8).setDepth(100)
	res = this.add.image(jues.x,jues.y,'res'); res.seguirGol = false;
	res2 = this.add.image(jues.x,jues.y,'res'); res2.seguirGol = false;
		
	jues.anim = this.tweens.chain({
		targets: jues,
		tweens:[
			{ texture: 'jues', onStart: retraso(moverJues,500), y:350, duration: 2000, ease: 'Back.easeOut' },
			{ onStart: juesSonido, scaleX: equipoA === true ? -0.8 : 0.8, duration: 10 },
			{ onStart: juesSonido, delay: 800, scaleX: equipoA === true ? 0.8 : -0.8, duration: 10 },
			{ delay: 500, texture: 'jues2' },
			{ onStart: ()=>{ estaEscena.time.addEvent({ delay: 400, callback: ()=> { desaparece.play(); }, paused: false }); }, delay: 500, y:-300, duration: 500, ease: 'Back.easeIn', onComplete: iniciarJuego }
		]
	});
		
	flecha = this.add.image(0, 0, 'flecha').setVisible(false).setOrigin(0, 0.5).setDepth(0).setScale(-0.4)

	pelota = this.physics.add.sprite(750, 370, 'pelota').setScale(5).setAlpha(0).setDepth(2).setBounce(0.3).setDrag(0.3).setDamping(true).setFriction(0.5).setCollideWorldBounds(true);
	pelota.body.isCircle = true; pelota.body.mass = 0.3;
	pelota.setCollideWorldBounds(); pelota.body.enable = false;
	
	mascara = this.make.graphics().fillCircle(100, 100, 100).setScale(0.18)
	pelota2 = this.add.tileSprite(0, 0,700,700, 'tilePelota').setMask(mascara.createGeometryMask()).setDepth(9).setScale(0.18).setAlpha(0)

	particulas();
	arcos = this.add.image(0, 0, 'arcos').setOrigin(0).setDisplaySize(cam.width, cam.height).setDepth(2)

	text = this.add.text(880, 20, 'RUBEN', { font: '35px Courier', fill: '#000000', stroke: '#ff0000', strokeThickness: 5, align: 'center' }).setOrigin(0.5);
	text.setDepth(100).setAlpha(0); text.posA = 420; text.posB = 1080;

	textGol1 = this.add.text(620, 30, '0', { font: '55px Courier', fill: '#ffc900', stroke: '#ff0000', strokeThickness: 5, align: 'center' }).setOrigin(0.5);
	textGol2 = this.add.text(875, 30, '0', { font: '55px Courier', fill: '#ffc900', stroke: '#ff0000', strokeThickness: 5, align: 'center' }).setOrigin(0.5);
	
	cartelGol = this.add.image(cam.width/2,cam.height/2,'cartelGol').setDepth(100).setScale(3,0)
	fabian = this.add.image(cam.width- 50,1500,'fabian'); fabian.x = cam.width-fabian.width/2; fabian.setDepth(10)
	amarilla = this.add.image(cam.width/2,cam.height,'amarilla').setOrigin(0.5,1).setScale(1.5).setDepth(99); amarilla.y = amarilla.y * 2;
	...
	cuadradoBlanco = this.add.image(cam.width/2,cam.height/2,'cuadradoBlanco').setDisplaySize(cam.width,cam.height).setDepth(111).setTint(0x000000).setAlpha(0)
	cartelFin = this.add.image(cam.width/2,cam.height/2-100,'cartelFin').setDepth(112).setScale(10,5).setAlpha(0)
	textFin = this.add.text(cam.width/2, cam.height/2-30, 'ROJO', { font: '70px arial', fill: '#ff0000', stroke: '#ffffff', strokeThickness: 3, align: 'center' }).setOrigin(0.5).setDepth(120).setAlpha(0).setScale(5)
...
function soltar(p) {
    ...
    // --- CAMBIO MULTIJUGADOR: Enviar disparo al oponente ---
    socket.emit('disparo', {
        idx: jIndex,
        px: p.x,
        py: p.y,
	...
	estaEscena.physics.moveToObject(pl, {x: p.x, y: p.y}, -dif)

	//cambiar el turno
	equipoA = !equipoA;
}

// ... (Resto del código: actualiza, golaso, iniciarMedioCampo, etc. sin cambios) ...
